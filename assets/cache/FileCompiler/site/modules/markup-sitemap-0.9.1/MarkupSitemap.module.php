...
\ProcessWire\wire('classLoader')->addNamespace('Thepixeldeveloper\Sitemap', \ProcessWire\wire("config")->paths->root . 'site/modules/markup-sitemap-0.9.1' . '/src/Sitemap');
\ProcessWire\wire('classLoader')->addNamespace('Rockett\Concerns', \ProcessWire\wire("config")->paths->root . 'site/modules/markup-sitemap-0.9.1' . '/src/Concerns');
\ProcessWire\wire('classLoader')->addNamespace('Rockett\Support', \ProcessWire\wire("config")->paths->root . 'site/modules/markup-sitemap-0.9.1' . '/src/Support');
...
class MarkupSitemap extends \ProcessWire\WireData implements \ProcessWire\Module
{
  ...
  public function ___install()
  {
    if (version_compare($this->config->version, '3.0.16') < 0) {
      throw new \ProcessWire\WireException('Requires ProcessWire 3.0.16+ to run.');
    }
  }
  ...
  public static function applyLanguageSupportHooks(): void
  {
    if (!static::$languageSupportHooksApplied) {
      foreach (['localUrl', 'localHttpUrl', 'localName'] as $pageHook) {
        ...
        \ProcessWire\wire()->addHook("Page::{$pageHook}", null, function ($event) use ($pageHookFunction) {
          \ProcessWire\wire('modules')->LanguageSupportPageNames->{$pageHookFunction}($event);
        });
      }
   ...
   * @return void
   */
  public function render(\ProcessWire\HookEvent $event): void
  {
    ...
    if (!$this->pages->get($rootPage) instanceof \ProcessWire\NullPage) {
  ...
  public function removeSitemapCache(): bool
  {
    ...
    if ($isCached && $cacheMethod === 'MarkupCache') {
      try {
        $removed = (bool) \ProcessWire\CacheFile::removeAll($this->cachePath, true);
      } catch (Exception $e) {
        $removed = false;
      }
    }
   ...
   * @return bool
   */
  protected function pageLanguageInvalid(\ProcessWire\Language $language, \ProcessWire\Page $page): bool
  {
   ...
   * @return void
   */
  protected function addLanguages(\ProcessWire\Page $page, Url $url): void
  {
   ...
   * @return string
   */
  protected function getLanguageIsoName(\ProcessWire\Language $language): string
  {
   ...
   * @return bool
   */
  public function canBeIncluded(\ProcessWire\Page $page, ?array $options): bool
  {
   ...
   * @return void
   */
  protected function addPagesFromRoot(\ProcessWire\Page $page): void
  {
   ...
   * @return void
   */
  protected function addAdditionalPages(\ProcessWire\Page $page, \ProcessWire\Language $language = null): void
  {
   ...
   * @return array
   */
  protected function ___getAdditionalPages(\ProcessWire\Page $page, \ProcessWire\Language $language = null): array
  {
    $return = [];
